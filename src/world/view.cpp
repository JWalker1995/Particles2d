...
void View::update_pixel(signed int x, signed int y, unsigned char r, unsigned char g, unsigned char b)
{
    ...
    cur_vc->data[dx.rem + dy.rem * VIEW_CHUNK_SIZE];
...
void View::draw()
{
    ...
    while (i2 != vcs.end())
    {
        ...
        glBegin(GL_QUADS);
        glTexCoord2f(0, 0); glVertex2i(i2->first.vcx    , i2->first.vcy    );
        glTexCoord2f(0, 1); glVertex2i(i2->first.vcx    , i2->first.vcy + 1);
        glTexCoord2f(1, 1); glVertex2i(i2->first.vcx + 1, i2->first.vcy + 1);
        glTexCoord2f(1, 0); glVertex2i(i2->first.vcx + 1, i2->first.vcy    );
        glEnd();
